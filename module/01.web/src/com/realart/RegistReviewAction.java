...
public class RegistReviewAction extends BaseAction implements UserInterface{
    ...
    public String execute() throws Exception {
        ...
        //判该名字和用户类型是否已存在
        boolean isExist = UserDao.isNameExist(name, USER_TYPE_REVIEW);
        if(isExist){
        ...
        User user = new User(name, USER_TYPE_REVIEW, password, certName, StringUtils.EMPTY, StringUtils.EMPTY,
                StringUtils.EMPTY, USER_STATE_NORMAL, StringUtils.EMPTY, date, time, getIp());
        UserDao.insertUser(user);

        //根据姓名和用户类型查用户
        user = UserDao.getUserByNameAndUserType(name, USER_TYPE_REVIEW);
        //创建压题图
