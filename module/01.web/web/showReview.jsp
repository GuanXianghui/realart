...
<div>
    <div>归类:<%=review.getType()%></div>
    <div>标题:<%=review.getTitle()%>
        <%
            if(isAdminLogin){
        %>
        <input type="button" value="<%=review.isZdtj()?"下置顶":"上置顶"%>" onclick="<%=review.isZdtj()?"zdtj(0)":"zdtj(1)"%>">
        <input type="button" value="<%=review.isZdtjTop()?"下置顶图":"上置顶图"%>" onclick="<%=review.isZdtjTop()?"zdtjTop(0)":"zdtjTop(1)"%>">
        <input type="button" value="<%=review.isMwgs()?"下美文":"上美文"%>" onclick="<%=review.isMwgs()?"mwgs(0)":"mwgs(1)"%>">
        <input type="button" value="<%=review.isMwgsTop()?"下美文图":"上美文图"%>" onclick="<%=review.isMwgsTop()?"mwgsTop(0)":"mwgsTop(1)"%>">
        <input type="button" value="<%=review.isBzyc()?"下本站":"上本站"%>" onclick="<%=review.isBzyc()?"bzyc(0)":"bzyc(1)"%>">
        <input type="button" value="<%=review.isBzycTop()?"下本站图":"上本站图"%>" onclick="<%=review.isBzycTop()?"bzycTop(0)":"bzycTop(1)"%>">
        <input type="button" value="<%=review.isHyzlTop()?"下会员图":"上会员图"%>" onclick="<%=review.isHyzlTop()?"hyzlTop(0)":"hyzlTop(1)"%>">
    ...
    <div>
        图片:<img src="<%=review.getPhoto()%>" width="25%">
        <%
            if(user != null && review.getUserId() == user.getId() && StringUtils.isNotBlank(review.getPhoto())){
                if(review.isSelfTop()){
        %>
        <input type="button" value="取消置顶" onclick="location.href='showReview.jsp?id=<%=id%>&setTop=0'">
        <%
                } else {
        %>
        <input type="button" value="置顶" onclick="location.href='showReview.jsp?id=<%=id%>&setTop=1'">
        <%
                }
            }
        %>
