...
<head>
    ...
    <script type="text/javascript" src="<%=baseUrl%>scripts/reviewUserRegistConfig.js"></script>
    <!-- 页面样式 -->
        ...
        <div class="content-box-content">
            <div class="tab-content default-tab">
                <span style="display: none;">
                    <form name="deleteRURIForm" action="deleteRURI.do" method="post">
                        <input type="hidden" name="token" value="<%=token%>">
                        <input type="hidden" name="ruri" id="delete_ruri">
                    </form>
                    <form name="saveRURIForm" action="saveRURI.do" method="post">
                        <input type="hidden" name="token" value="<%=token%>">
                        <textarea name="ruri" id="save_ruri"></textarea>
                    </form>
                </span>
                <form>
                    <table id="ruri_table">
                        ...
                        <tr>
                            <td>
                                <input class="text-input large-input" type="text" id="ruri_name_<%=i+1%>" value="<%=name%>">
                            </td>
                            <td>
                                <select class="text-input large-input" id="ruri_need_<%=i+1%>">
                                    <option value="1"<%="1".equals(need)?" selected":""%>>必输</option>
                                    <option value="2"<%="2".equals(need)?" selected":""%>>非必输</option>
                                </select>
                            </td>
                            <td>
                                <select class="text-input large-input" id="ruri_type_<%=i+1%>">
                                    <option value="1"<%="1".equals(type)?" selected":""%>>字符串</option>
                                    <option value="2"<%="2".equals(type)?" selected":""%>>图片/文件</option>
                                </select>
                            </td>
                            <td>
                                <input class="button" type="button" onclick="deleteRURI(<%=i+1%>);" value="删除">
                            </td>
                        </tr>
                        <%
                            }
                ...
                <input class="button" type="button" onclick="addRURI();" value="新增注册项">
                <input class="button" type="button" onclick="saveRURI();" value="保存">
            </div>
        </div>
    </div>
    <div class="clear"></div>
